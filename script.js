...
function jogoCompativel(jogo) {
  ...
  const gpuScore = GPUS.find(g => g.id === state.hardware.gpu)?.score ?? 0;
  const ramScore = MEMORIAS.find(m => m.id === state.hardware.ram)?.score ?? 0;
  const scorePC = cpuScore + gpuScore + ramScore;
  
  const requisitosCPU = jogo.cpu ?? 2;
  const requisitosgpu = jogo.gpu ?? 1.0;
  const requisitoRam = jogo.ram ?? 2;
  
  const compativel = (cpuScore >= requisitosCPU) && (gpuScore >= requisitosgpu) && (ramScore >= requisitoRam);
  return compativel;
}

function filtrarJogos() {
  let resultado = JOGOS_COMPLETOS.filter(j => {
    const hardwareMatch = jogoCompativel(j);
    const buscaMatch = j.nome.toLowerCase().includes(state.busca.toLowerCase()) || 
                       j.codigo.includes(state.busca.toUpperCase());
    
    const statusExpandido = STATUS_MAP[j.status] || j.status;
    let statusMatch = true;
    if (state.filtroStatus !== 'todos') {
      const filtroExpandido = STATUS_MAP[state.filtroStatus] || state.filtroStatus;
      statusMatch = statusExpandido === filtroExpandido;
    }
...
framerate_limit = 60`;
  ...
  const conteudo = `# ============================================================================
# Configuração Xenia Canary - ${tipoPC.toUpperCase()}
# ============================================================================
...
# INSTRUÇÕES DE USO:
# ============================================================================
# 1. Salve este arquivo como: xenia-canary.config.toml
# 2. Coloque em: C:\\Users\\[SeuUsuário]\\AppData\\Local\\xenia-canary\\
# 3. Reinicie o Xenia Canary
# 4. Se tiver problemas, reduza resolution_scale em 0.25x
# ============================================================================`;
...
Nome: xenia_${tipoPC.toLowerCase()}.toml

📁 Salve em:
C:\\Users\\[SeuUsuário]\\AppData\\Local\\xenia-canary\\
...
function render() {
  ...
  root.innerHTML = `
    ...
    <div class="section">
      <h2>🔍 Buscar Jogos</h2>
      <input type="text" class="search-box" placeholder="Buscar por nome ou código Xbox..." 
             value="${state.busca}" onchange="updateBusca(this.value)" oninput="updateBusca(this.value)">
    </div>
    ...
    ${state.jogoSelecionado ? renderModal(state.jogoSelecionado) : ''}
  `;
}

function renderModal(jogo) {
  return `
      ...
      <div class="modal-content">
        ...
        <div class="modal-section">
          ...
          <div style="padding: 10px;">
            <div>📄 Código: ${jogo.codigo}</div>
            <div>📂 Categoria: ${jogo.categoria}</div>
            <div>⭐ Status: ${STATUS_TEXT_MAP[jogo.status] || '❓ Desconhecido'}</div>
            <div>🎮 FPS: ${jogo.fps}</div>
            <div>💾 RAM: ${jogo.ram}GB | GPU: ${jogo.gpu}GB | CPU: ${jogo.cpu} cores</div>
          </div>
        </div>
        ...
        ${jogo.comentarios && jogo.comentarios.length > 0 ? `
          <div class="modal-section">
            ...
            <ul>
              ${jogo.comentarios.slice(0, 3).map(c => `<li>${c}</li>`).join('')}
            </ul>
          </div>
        ` : ''}
